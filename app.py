...
def home():
    ...
    body=f'''<header class=top><div class=brand><div class=logo>H</div><div><h1>Hysteria Control</h1><small>Панель управления сервером</small></div></div><div class=top-actions><a href=/logout><button>Выйти</button></a><button class=theme id=themeToggle title="Сменить тему">◐</button></div></header>
...
def resources():
    ...
    cpu=cpu_counters(); load=float(open('/proc/loadavg').read().split()[0]); n=cpu[0]-cpu[1]; cpu_pct=0 if not last_cpu or n==last_cpu[0]-last_cpu[1] else max(0,min(100,100*(1-(cpu[1]-last_cpu[1])/max(1,n-(last_cpu[0]-last_cpu[1])))))
    last_cpu=cpu
    result={'load':f'{load:.2f}','cpu':f'{cpu_pct:.0f}%', 'ram':f'{(total-avail)/1024:.0f} / {total/1024:.0f} МБ','swap':f'{(st-sf)/1024:.0f} / {st/1024:.0f} МБ','disk':f'{du/1024:.0f} / {dt/1024:.0f} МБ','resources':{'ram':{'percent':round(100*(total-avail)/max(1,total))},'swap':{'percent':round(100*(st-sf)/max(1,st))},'disk':{'percent':int(d[4].rstrip('%'))}}}
    return result
@app.get('/api/metrics')
def metrics():
    if not logged(): abort(403)
    ...
    data['users']={r['username']:{'online':int(online.get(r['username'],0)) if r['enabled'] else 0,'tx':int(r[2]),'rx':int(r[3]),'enabled':bool(r['enabled'])} for r in rows}
    return data
@app.get('/logout')
